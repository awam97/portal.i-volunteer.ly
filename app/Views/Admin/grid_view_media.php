                    ...
                    <div class="col-md-3 col-xs-6 <?= $entityName; ?>-item" data-id="<?= $entity->id; ?>">
                        <div class="media-item">
                            <div class="media-thumbnail">
                                ...
                                $uploadDir = 'https://portal.i-volunteer.ly/uploads/' . $entityName . '_files/';
                                $filePath = null;
                                ...
                                foreach (['png', 'jpg', 'jpeg', 'gif', 'pdf'] as $ext) {
                                    $fileName = $entity->id . '.' . $ext; // Construct file name                                    
                                    $visiblefileName = $entity->filename ; // Construct file name
                                    $potentialFilePath = $uploadDir . $fileName; // Full file path
                                    $placeholder = $placeholderImage;
                                    $headers = @get_headers($potentialFilePath, 1); // Get file headers
                                    if ($headers && strpos($headers[0], '200')) {
                                        // Check MIME type in headers
                                        $mimeType = $headers['Content-Type'] ?? null;
                                        if (in_array($mimeType, $allowedImageMimeTypes)) {
                                            $filePath = $potentialFilePath; // Valid image found
                                            $placeholder = $filePath;
                                            break;
                                        } elseif ($mimeType === 'application/pdf') {
                                            $filePath = $potentialFilePath; // PDF found
                                            $placeholder = $placeholderPDF; // Use PDF placeholder
                                            break;
                                        }
                                    }
                                }
                                ?>
                                <a href="<?php echo $potentialFilePath;?>" ><img src="<?= $placeholder; ?>" alt="<?= $entity->name ?? 'Entity'; ?>"></a>
                            </div>
                            <div class="media-details">
                                <p><b><?= $visiblefileName; ?></b></p>
                                <div class="media-actions">
                                    <label class="col-md-2 select-checkbox">
                                        <input class="entity-checkbox" type="checkbox" name="entity" data-id="<?= $entity->id; ?>" value="<?= $entity->id; ?>">
                                        <span></span>
                                    </label>
                                    <a href="#tab3" class="col-md-5 btn btn-danger btn-edit btn-lg" data-toggle="tab" onclick="editEntity(<?= $entity->id; ?>)">تعديل</a>
                                    <button class="col-md-5 btn btn-info btn-delete btn-lg" data-id="<?= $entity->id; ?>">حذف</button>                                    
                                </div>
                            </div>
                        </div>
                    </div>
                                                ...
                                                document.addEventListener('DOMContentLoaded', function () {
                                                    $('#$summernote_id').summernote({
                                                        ...
                                                        toolbar: [
                                                            ...
                                                            ['insert', ['link', 'customMediaButton']], // إضافة زر مخصص
...
<script>
    $(document).ready(function () 
    {
        ...
        $(Addform).on('submit', function (e) {
            ...
            if (fileInput && fileInput.files.length > 0) {
                ...
                reader.onload = function (e) {
                    ...
                    // Add the original file name to the form data
        ...
        function sendData(formData) {
            $.ajax({
                url: '<?= base_url("Admin/update_entity") ?>',
                type: 'POST',
